...
public class Transcations extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b1){
            ...
            String pinn = JOptionPane.showInputDialog("Enter Pin");//For input Pin for balance enquiry
            conn c1 = new conn();
            try{
            ResultSet rs = c1.s.executeQuery(" SELECT balance FROM Bank ORDER BY pin = '"+pinn+"' DESC LIMIT 1");
           if(rs.next()){
            String balance = rs.getString("balance");
            JOptionPane.showMessageDialog(null,"Your Account Balance is" +balance);//For showing Output
        }
        }catch(Exception e){
            e.printStackTrace();
        }
